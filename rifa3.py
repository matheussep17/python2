...
def criar_tabela_rifas():
    conn = sqlite3.connect("rifas2.db")
    cursor = conn.cursor()
    cursor.execute("""
...
def vender_rifa(numero_rifa, nome, telefone, endereco):
    ...
    if rifa:
        print(f"Desculpe a rifa de número {numero_rifa}, já foi vendida.")
        print("--------------------------")
...
# Função para limpar todas as rifas do banco de dados
def limpar_rifas():
    conn = sqlite3.connect("rifas2.db")
    cursor = conn.cursor()
...
def sortear_rifa():
    ...
    if rifas:
        ...
        print(f"A rifa sorteada é a número {numero_rifa_sorteada}.")

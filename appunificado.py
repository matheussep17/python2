# app_unificado.py
# Único executável com três módulos: Conversor, YouTube (download) e Transcrição (DOCX)
# Mantém layout darkly e a lógica de cada app original.
...
# =============== Módulo 1: Conversor de Vídeo/Imagem =================
...
class ConverterFrame(ttk.Frame):
    ...
    def _build_ui(self):
        ttk.Label(self, text="Conversor de Vídeo / Imagem", font=("Helvetica", 20, "bold")).pack(pady=(0, 10))

        row = ttk.Frame(self); row.pack(pady=5, fill="x")
        ...
        if HAS_DND:
            ttk.Label(self,
                      text="Dica: arraste e solte arquivos de vídeo/áudio (mp4, mkv, mp3...) ou imagem (jpg, png, cr2...)",
                      font=("Helvetica", 10, "italic"), foreground="#9aa0a6").pack(anchor="w", pady=(0,6))

        self.fmt_row = ttk.Frame(self)
        ...
        self.format_menu = ttk.Combobox(self.fmt_row, textvariable=self.formato_destino, values=[], state="readonly")
        self.format_menu.pack(side="left", padx=(10,0))

        btn_row = ttk.Frame(self); btn_row.pack(pady=(10,6), fill="x")
        ...
        self.status_label = ttk.Label(self, textvariable=self.status_var, font=("Helvetica", 11))
        self.status_label.pack(anchor="w")

        self.open_btn = ttk.Button(self, text="Abrir pasta do arquivo convertido", command=self.abrir_pasta,
...
# =============== Módulo 2: Transcrição (para .docx) =================
...
class TranscriberFrame(ttk.Frame):
    ...
    def _build_ui(self):
        ttk.Label(self, text="Transcritor de Áudio (Word)", font=("Helvetica", 20, "bold")).pack(pady=(0,10))

        row = ttk.Frame(self); row.pack(pady=5, fill="x")
        ttk.Button(row, text="Selecionar Arquivo(s)", command=self.selecionar_arquivos, bootstyle=WARNING).pack(side="left")
        ttk.Button(row, text="🗑 Remover arquivo(s)", command=self.remover_arquivos, bootstyle=DANGER).pack(side="left", padx=(10,0))

        self.label_sel = ttk.Label(self, text="Nenhum arquivo selecionado", font=("Helvetica", 12))
        self.label_sel.pack(anchor="w", pady=(8,0))

        btns = ttk.Frame(self); btns.pack(pady=(10,6), fill="x")
        ...
        self.progress = ttk.Progressbar(self, orient=tk.HORIZONTAL, length=400, mode="determinate",
                                        variable=self.progress_var, maximum=100)
        self.progress.pack(fill="x", pady=(6,2))
        self.status_lbl = ttk.Label(self, textvariable=self.status_var, font=("Helvetica", 11))
        self.status_lbl.pack(anchor="w")

        self.btn_open = ttk.Button(self, text="Abrir pasta do último .docx", command=self.abrir_pasta,
                                   bootstyle=INFO, state=DISABLED)
        self.btn_open.pack(pady=10)

    def selecionar_arquivos(self):
        tipos = [("Áudio/Vídeo", "*.mp3 *.wav *.m4a *.mp4 *.mkv *.mov *.webm"),
                 ("Áudio", "*.mp3 *.wav *.m4a"),
...
# =============== Módulo 3: YouTube (baixar música/vídeo) =============
...
class YouTubeFrame(ttk.Frame):
    ...
    def _build_ui(self):
        ttk.Label(self, text="Baixar Vídeos e Músicas do YouTube", font=("Helvetica", 22, "bold")).grid(row=0, column=0, columnspan=4, pady=(0,10), sticky="w")

        ttk.Label(self, text="YouTube URL:", font=("Helvetica", 14)).grid(row=1, column=0, padx=5, pady=5, sticky="e")
        self.url_entry = ttk.Entry(self, width=60, font=("Helvetica", 12))
        self.url_entry.grid(row=1, column=1, columnspan=3, padx=5, pady=5, sticky="we")

        ttk.Button(self, text="Escolher pasta de destino", command=self.choose_dest_folder, bootstyle=SUCCESS)\
            .grid(row=2, column=0, padx=5, pady=5, sticky="e")
        self.dest_label = ttk.Label(self, text=self.destination_folder or "Nenhuma pasta selecionada",
                                    width=50, anchor="w", font=("Helvetica", 12))
        self.dest_label.grid(row=2, column=1, columnspan=3, padx=5, pady=5, sticky="we")

        ttk.Label(self, text="Formato:", font=("Helvetica", 14)).grid(row=3, column=0, padx=5, pady=5, sticky="e")
        self.format_menu = ttk.Combobox(self, textvariable=self.selected_format, values=["Música", "Vídeo"],
        ...
        self.quality_label = ttk.Label(self, text="Qualidade do vídeo:", font=("Helvetica", 14))
        self.quality_label.grid(row=3, column=2, padx=5, pady=5, sticky="e")
        self.quality_menu = ttk.Combobox(self, textvariable=self.selected_quality,
        ...
        actions = ttk.Frame(self); actions.grid(row=4, column=0, columnspan=4, pady=(15,8), sticky="ew")
        actions.columnconfigure(0, weight=1); actions.columnconfigure(1, weight=1)
        self.download_btn = ttk.Button(actions, text="Baixar", command=self.start_download, bootstyle=PRIMARY)
        self.download_btn.grid(row=0, column=0, sticky="ew", padx=(0,5))
        self.cancel_btn = ttk.Button(actions, text="Cancelar", command=self.cancel_download, bootstyle=SECONDARY, state=DISABLED)
        ...
        self.open_folder_button = ttk.Button(self, text="Abrir local do arquivo", command=self.open_file_location,
                                             bootstyle=INFO, state=DISABLED)
...
class SuperApp(ttk.Window if not HAS_DND else TkinterDnD.Tk):
    def __init__(self):
        if HAS_DND:
            super().__init__()
            self.title("Midia Suite - Conversor • YouTube • Transcrição")
            self.style = ttk.Style(theme="darkly")
            self.geometry("980x560")
        else:
            super().__init__(title="Midia Suite - Conversor • YouTube • Transcrição",
                             themename="darkly", size=(980, 560))
    ...
    def _build_shell(self):
        ...
        # Sidebar
        side = ttk.Frame(self, padding=16); side.grid(row=0, column=0, sticky="ns")
        ttk.Label(side, text="Midia Suite", font=("Helvetica", 22, "bold")).pack(pady=(0,12))
        ttk.Button(side, text="Conversor", bootstyle=PRIMARY, command=lambda: self._show("converter"),
                   width=20).pack(pady=6, fill="x")
        ttk.Button(side, text="YouTube (Baixar)", bootstyle=INFO, command=lambda: self._show("youtube"),
                   width=20).pack(pady=6, fill="x")
        ttk.Button(side, text="Transcrição (Word)", bootstyle=SUCCESS, command=lambda: self._show("transcribe"),
                   width=20).pack(pady=6, fill="x")
        ...
        # Stacked frames
        self.frames = {
            "converter": ConverterFrame(self.content),
            "youtube": YouTubeFrame(self.content),
            "transcribe": TranscriberFrame(self.content),
        }

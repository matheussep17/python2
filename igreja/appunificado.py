...
# UI com topbar, sidebar, cards e statusbar.
# Lazy imports p/ iniciar rápido e evitar travamentos ao empacotar.
...
# Checagens (sem importar pesado no boot)
HAS_RAWPY = _has_module("rawpy")                   # só importamos quando necessário
HAS_FW    = _has_module("faster_whisper")          # idem (modelo whisper é pesado)
HAS_DOCX  = _has_module("docx")                    # idem (python-docx)
...
class TranscriberFrame(ttk.Frame):
    ...
    def _save_docx(self, full_text, out_docx, info):
        try:
    ...
    def _transcribe_one(self, in_path, out_docx, idx, total):
        try:
            ...
            if not self._save_docx(full_text, out_docx, info):
                return False
...
class YouTubeFrame(ttk.Frame):
    def __init__(self, master, on_status):
        ...
        self.selected_format = tk.StringVar(value="Música")  # Música ou Vídeo
        self.selected_quality = tk.StringVar(value="best")
        self.downloaded_file = None
        self.cancel_requested = False
        self._last_tmp_file = None
        self._yt_dlp = None  # setado no start_download()
    ...
    def _build_ui(self):
        ...
        self.quality_menu = ttk.Combobox(
            opts, textvariable=self.selected_quality,
    ...
    def start_download(self):
        ...
        # Lazy import yt_dlp (evita peso no startup)
        try:
            import yt_dlp
            self._yt_dlp = yt_dlp
        except Exception:
    ...
    # --- MÉTODO ATUALIZADO: respeita qualidade exata -> <= alvo -> fallback best ---
    def download_media(self, url, fmt_mode, quality_choice):
        try:
            yt_dlp = self._yt_dlp  # já carregado no start_download()
            ...
            h = parse_height(quality_choice)

            # Base comum
            common_args = {
                "extractor_args": {
                    "youtube": {
                        "player_client": ["android"],  # evita clients web que estouram SABR
            ...
            if fmt_mode == "Música":
                ...
                # Vídeo
                if h is None:
                    # "best" explícito – preferir MP4 quando possível
                    fmt = "bestvideo[ext=mp4][vcodec!=none]+bestaudio[ext=m4a]/bestvideo[vcodec!=none]+bestaudio/best[ext=mp4]/best"
                    ydl_opts = {
                        **common_args,
                        "format": fmt,
                    ...
                    # 1) Tentar ALTURA EXATA (video+audio)
                    exact_pair = f"bestvideo[height={h}][vcodec!=none]+bestaudio/best[height={h}]"
                    # 2) Tentar MELHOR <= ALTURA
                    le_pair = f"bestvideo[height<={h}][vcodec!=none]+bestaudio/best[height<={h}]"
                    # 3) Fallbacks gerais
                    fallbacks = "best[ext=mp4]/best"

                    fmt = f"{exact_pair}/{le_pair}/{fallbacks}"

                    ydl_opts = {
                        **common_args,
                        "format": fmt,
                        "merge_output_format": "mp4",
                        "format_sort": [f"res:{h}", "fps", "vcodec:h264"],
                        "format_sort_force": False,
                    }
...
class SuperApp(ttk.Window if not HAS_DND else TkinterDnD.Tk):
    def __init__(self):
        ...
        if HAS_FW and HAS_DOCX:
            ...
            ttk.Button(sidebar, text="📝  Transcrição (instale deps)", state=DISABLED).pack(pady=6, fill="x")
    ...
    def _show(self, key):
        ...
        f.lift()
        if key == "converter":
            self.title_label.config(text=" — Conversor")
        elif key == "youtube":
            self.title_label.config(text=" — YouTube")
        else:
            self.title_label.config(text=" — Transcrição")

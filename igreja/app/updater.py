...
def schedule_windows_self_replace(downloaded_exe: Path) -> None:
    ...
    script = "\n".join(
        [
            ...
            "  Write-Log (\"Tentativa {0} de substituir o executavel.\" -f $attempt)",
            "  try {",
            ...
            "    }",
            "    Move-Item -LiteralPath $Package -Destination $Target -Force -ErrorAction Stop",
            ...
            "      }",
            "    } catch {",
            "      Write-Log (\"Falha ao limpar destino apos erro: {0}\" -f $_.Exception.Message)",
            "    }",
            "    try {",
            "      if (Test-Path -LiteralPath $Backup) {",
